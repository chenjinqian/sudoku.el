...
(defun my_last (lst)
  (car (my_rev lst)))
(defun exlast (lst)
  (my_rev (cdr (my_rev lst))))
(defun pecons (lsta lstb)
  (if (and (atom lsta) (not (equal lsta nil)))
      nil
      (if (null lsta)
          lstb
          (pecons (exlast lsta) (cons (my_last lsta) lstb)))))
(defun naft (lst n)
  (if (eq n 1)
      lst
      (naft (cdr lst) (- n 1))))
(defun my_nbef (lst n)
  ...
  (nbefacc lst n 1))
(defun find_acc (place list acc)
  (if (null list)
      0
      (if (equal (caar list) place)
          acc
          (find_acc place (cdr list) (+ 1 acc)))))
(defun my_find (place)
  (find_acc place lists 1))

(defun rep (pla pos)
  (if (equal (my_find pla) 0)
      (setq lists
            (cons (cons pla pos) lists))
      (setq lists (pecons
                   (my_nbef lists (my_find pla))
                   (cons (cons pla pos)
                         (cdr (naft lists (my_find pla))))))))
...
(defun consider (pla)
  (if (null pla)
      ...
      (if (simp (cel pla))
          nil
          (if (not (simp (same (difs (cel pla)
                                     (cels (za pla)))
                               (same (difs (cel pla)
                                           (cels (zb pla)))
                                     (difs (cel pla)
                                           (cels (zc pla)))))))
              (rep pla (difs (cel pla)
                             (cels (sure (rela pla)))))
